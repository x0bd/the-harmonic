...
export const AudioPlayer = ({ duration, title }: { duration?: string; title: string }) => {
    ...
    return (
        <div className="w-full bg-[#0a0a0c] border border-white/10 p-6 md:p-8 rounded-[2rem] flex flex-col md:flex-row items-center gap-8 shadow-[0_30px_60px_rgba(0,0,0,0.6),inset_0_2px_4px_rgba(255,255,255,0.02)] relative overflow-hidden group mb-16">
            ...
            >
                {/* Visual LED */}
                <div className={`absolute top-2 right-3 w-2 h-2 rounded-full transition-colors duration-300 ${isPlaying ? 'bg-accent shadow-[0_0_12px_#ff6000]' : 'bg-white/20'}`}></div>
            ...
            <div className="flex-grow w-full relative z-10">
                <div className="flex justify-between items-end mb-3">
                    <span className="font-mono text-[10px] text-accent uppercase tracking-[0.3em] font-bold flex items-center gap-2">
                        {isPlaying && <span className="w-1.5 h-1.5 bg-accent animate-pulse rounded-sm"></span>}
                        {isPlaying ? 'TRANSMITTING' : 'AUDIO_ARCHIVE'}
                    </span>
                    <span className="font-mono text-[10px] text-foreground/40 tracking-[0.2em]">{isPlaying ? 'PLAYING...' : 'PAUSED'} // {duration || '00:00'}</span>
                </div>
                ...
                <div className="w-full h-12 flex items-end gap-[3px] opacity-80 cursor-pointer hover:opacity-100 transition-opacity">
                    {Array.from({ length: 70 }).map((_, i) => {
                        ...
                        const isPlayed = isPlaying && i < 20; // Fake played progress
                        return (
                           <div 
                               key={i} 
